...
class Blocks {
  ...
  private async $getBlockExtended(block: IEsploraApi.Block, transactions: TransactionExtended[]): Promise<BlockExtended> {
    ...
    if (block.height === 0) {
      ...
      extras.segwitTotalWeight = stats.swtotal_weight ?? (stats.total_size ? stats.total_size * 4 : 0);
    }
    ...
    if (['mainnet', 'testnet', 'signet', 'testnet4', 'bitcoincash', 'bitcoincashtestnet'].includes(config.MEMPOOL.NETWORK)) {
      let pool: PoolTag;
  ...
  public async $getBlock(hash: string, skipMemoryCache: boolean = false): Promise<BlockExtended | IEsploraApi.Block> {
    ...
    if (['mainnet', 'testnet', 'signet', 'testnet4', 'bitcoincash', 'bitcoincashtestnet'].includes(config.MEMPOOL.NETWORK) === false) {
  ...
  public async $getBlocksBetweenHeight(fromHeight: number, toHeight: number): Promise<any> {
    ...
    while (fromHeight <= toHeight) {
      ...
      if (Common.isBitcoinCash()) {
        cleanBlock.segwit_total_txs = cleanBlock.segwit_total_txs ?? 0;
        cleanBlock.segwit_total_size = cleanBlock.segwit_total_size ?? 0;
        cleanBlock.segwit_total_weight = cleanBlock.segwit_total_weight ?? (cleanBlock.size ? cleanBlock.size * 4 : 0);
      }
  ...
  public async $getBlockAuditSummary(hash: string): Promise<BlockAudit | null> {
    if (['mainnet', 'testnet', 'signet', 'testnet4', 'bitcoincash', 'bitcoincashtestnet'].includes(config.MEMPOOL.NETWORK) && Common.auditIndexingEnabled()) {
